  ...
  >
    <v-list>
      ...
      >
        ...
        >
          <v-list-item-content>
            <v-list-item-title v-text="post._id"></v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list-item-group>
    </v-list>
...
export default {
  ...
  methods: {
    submit() {
        ...
        this.axios
        .get(
          process.env.VUE_APP_BACKEND_URL + process.env.VUE_APP_SEARCH_HASHTAG_ENDPOINT + this.search
        ).then((response) => {
           if (response.data)
